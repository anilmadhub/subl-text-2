    ...
    def check_status(self, path):
        ...
        for line in result:
            if len(line) < 2:
                continue
            path_without_root = path.replace(self.root_dir + '\\', '', 1)
            path_without_root_backslash = path_without_root.replace('\\', '/')
            path_regex = re.escape(path_without_root) + '$'
            path_regex_backslash = re.escape(path_without_root_backslash) + '$'
            search_result = re.search(path_regex, line)
            search_result_backslash = re.search(path_regex_backslash, line)
            if self.root_dir != path and (search_result == None and
                    search_result_backslash == None):
                continue
